...
/*   Updated: 2024/07/07 19:32:38 by tkubanyc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*routine_general(void *param)
{
	...
	while (get_value_int(&data->start_ready_mtx, &data->start_ready) != 1)
		;
	start_time = get_value_long(&data->start_time_mtx, &data->start_time);
	if (start_time == -1)
		return (NULL);
	if (!set_value_long(&philo->time_last_eat_mtx, &philo->time_last_eat, start_time))
		return (NULL);
